...
function Header() {
  ...
  return (
    <header
      className={`sticky top-0 left-0 w-full z-50 transition-transform duration-300 backdrop-blur-md bg-white/50 shadow-md h-16 ${showHeader ? 'translate-y-0' : '-translate-y-full'
        }`}
      role="banner"
    >
            ...
            {isLoggedIn && isSuperuser && (
              <li>
                <div className="relative">
                  ...
                  >
                    Admin Portal
                    <ChevronDown className={`ml-1 w-4 h-4 transform ${adminMenuOpen ? 'rotate-180' : ''}`} />
                  </button>
                  {adminMenuOpen && (
                    <ul
                      id="admin-dropdown"
